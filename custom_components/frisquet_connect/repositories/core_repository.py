import logging
import aiohttp
from custom_components.frisquet_connect.domains.exceptions.call_api_exception import (
    CallApiException,
)
from custom_components.frisquet_connect.domains.exceptions.forbidden_access_exception import (
...
async def _async_call_api(url, method: str, params: dict = None, data_json: dict = None) -> dict:
    ...
    async with aiohttp.ClientSession(headers=DEFAULT_HEADERS) as session:
        try:
            if method == "GET":
                response = await session.get(url, headers=DEFAULT_HEADERS, params=params)
            elif method == "POST":
                headers["Content-Type"] = "application/json"
                response = await session.post(url, headers=DEFAULT_HEADERS, params=params, json=data_json)
            else:
